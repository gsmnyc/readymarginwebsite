...
const production = (process.env.VERCEL_ENV || process.env.SITE_ENV) === "production";
...
if (!enabled || !production) {
  ...
  let origin;
  try {
    origin = new URL(process.env.SITE_URL || "https://www.readymargin.com");
  } catch {
    console.warn("IndexNow skipped because SITE_URL is invalid.");
  }
  if (origin) {
