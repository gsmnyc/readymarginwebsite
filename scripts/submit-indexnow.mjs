...
if (!enabled || !production) {
  ...
  if (origin.protocol !== "https:" || origin.hostname !== "readymargin.com") {
    console.warn(
      "IndexNow skipped because SITE_URL is not the canonical HTTPS host.",
    );
  } else {
    ...
    const search = JSON.parse(
      readFileSync(
        new URL("../content/search-pages.json", import.meta.url),
        "utf8",
      ),
    );
    ...
    const urlList = [
      origin.origin,
      ...[...canonicalPaths].map((path) => origin.origin + path),
    ];
    const payload = {
    ...
    if (process.env.INDEXNOW_DRY_RUN === "true") {
      ...
      try {
        ...
        if (response.status === 200 || response.status === 202)
          ...
          console.warn(
            `IndexNow did not accept this notification (${response.status}).`,
          );
      } catch {
        console.warn(
          "IndexNow notification could not be sent; the build will continue.",
        );
      }
    }
  }
