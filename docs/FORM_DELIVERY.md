...
6. Submit a clearly labelled test enquiry from the live form. Confirm one Sheet row, one notification and a success state. Retry the same unchanged enquiry on the same UTC day and confirm no duplicate row. Remove only the test row afterward.
...
The receiver uses a script lock and a receipt column to deduplicate retries. Identical validated submissions on the same UTC day have the same receipt; an edited submission or a new day creates a new enquiry. Email delivery is separate from durable acceptance: a mail failure leaves the saved row Pending and does not ask the visitor to re-enter it. Notifications are best-effort and may repeat if Google sends a message but the following status write fails; exactly-once email delivery is not claimed.
